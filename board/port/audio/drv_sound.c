...
void SAIA_samplerate_set(rt_uint32_t freq)
{
    ...
    PeriphClkInitStruct.PeriphClockSelection = RCC_SAI23CLKSOURCE_PLL3;
    PeriphClkInitStruct.PLL3.PLL3P           = SAI_PSC_TBL[i][1];
...
rt_err_t SAIA_config_init(void)
{
    ...
    _sai_a.hsai.Instance = SAI1_Block_B;

    _sai_a.hsai.Init.AudioMode      = SAI_MODEMASTER_TX;        //配置为发送模式
    _sai_a.hsai.Init.Synchro        = SAI_ASYNCHRONOUS;         //模块内部为异步
    _sai_a.hsai.Init.OutputDrive    = SAI_OUTPUTDRIVE_ENABLE;   //立刻输出
    _sai_a.hsai.Init.NoDivider      = SAI_MASTERDIVIDER_ENABLE; //使能MCK输出NOACK
    _sai_a.hsai.Init.FIFOThreshold  = SAI_FIFOTHRESHOLD_1QF;    //1/4FIFO
    _sai_a.hsai.Init.MonoStereoMode = SAI_STEREOMODE;
    _sai_a.hsai.Init.AudioFrequency = _usAudioFreq;
    _sai_a.hsai.Init.Protocol       = SAI_FREE_PROTOCOL;            //设置SAI1协议为:自由协议(支持I2S/LSB/MSB/TDM/PCM/DSP等协议)
    _sai_a.hsai.Init.DataSize       = SAI_DATASIZE_16;              //设置数据大小
    _sai_a.hsai.Init.FirstBit       = SAI_FIRSTBIT_MSB;             //数据MSB位优先
    _sai_a.hsai.Init.ClockStrobing  = SAI_CLOCKSTROBING_RISINGEDGE; //数据在时钟的上升/下降沿选通

    //帧设置
    _sai_a.hsai.FrameInit.FrameLength       = 32;
    _sai_a.hsai.FrameInit.ActiveFrameLength = 16;
    _sai_a.hsai.FrameInit.FSDefinition      = SAI_FS_CHANNEL_IDENTIFICATION; //FS信号为SOF信号+通道识别信号
    _sai_a.hsai.FrameInit.FSPolarity        = SAI_FS_ACTIVE_LOW;             //FS低电平有效(下降沿)
    _sai_a.hsai.FrameInit.FSOffset          = SAI_FS_BEFOREFIRSTBIT;         //在slot0的第一位的前一位使能FS,以匹配飞利浦标准

    //SLOT设置
    _sai_a.hsai.SlotInit.FirstBitOffset = 0;                                   //slot偏移(FBOFF)为0
    _sai_a.hsai.SlotInit.SlotSize       = SAI_SLOTSIZE_16B;                    //slot大小为32位
    _sai_a.hsai.SlotInit.SlotNumber     = 2;                                   //slot数为2个
    _sai_a.hsai.SlotInit.SlotActive     = SAI_SLOTACTIVE_0 | SAI_SLOTACTIVE_1; //使能slot0和slot1

    HAL_SAI_Init(&_sai_a.hsai); //初始化SAI
    //  HAL_SAI_InitProtocol(&h_sai, SAI_I2S_STANDARD, _usWordLen, 2);//2--left_channel and right_channel
    SAI1_Block_B->CR1 |= 1 << 17; //写入CR1寄存器中
    __HAL_SAI_ENABLE(&_sai_a.hsai);
...
rt_err_t SAIA_tx_dma(void)
{
    ...
    _sai_a.hdma.Instance                 = DMA1_Stream2;
    _sai_a.hdma.Init.Request             = DMA_REQUEST_SAI1_B;
    ...
    _sai_a.hdma.Init.MemBurst            = DMA_MBURST_SINGLE; //存储器单次突发传输
    _sai_a.hdma.Init.PeriphBurst         = DMA_PBURST_SINGLE; //外设突发单次传输

    __HAL_LINKDMA(&_sai_a.hdma, hdmatx, _sai_a.hdma); //将DMA与SAI联系起来
    HAL_DMA_Init(&_sai_a.hdma);                       //初始化DMA

    __HAL_DMA_ENABLE_IT(&_sai_a.hdma, DMA_IT_TC);
    HAL_DMAEx_MultiBufferStart(&_sai_a.hdma, (uint32_t)buffer0, (uint32_t)&SAI1_Block_B->DR, (uint32_t)buffer1, num); //开启双缓冲

    HAL_NVIC_SetPriority(DMA1_Stream2_IRQn, 0, 0); //DMA中断优先级
